...
```mermaid
graph TD
    A["매시간 실행 트리거"] --> B{"알림 수신: Competitor Price Alert"}
    B -- "알림 없음" --> C["워크플로우 즉시 중지"]
    B -- "알림 감지" --> D["사내 스토리지 커넥터: 재고 및 원가 마진 조회"]
    D --> E{"마진 >= 10% & 재고 충분?"}
    E -- "No 거절" --> F["매칭 불가 PDF 보고서 생성"]
    F --> G["담당자 알림 이메일 발송"]
    E -- "Yes 승인" --> H["판매 20% 증가 가정 재고 보충 계획 수립"]
    H --> I["소셜 미디어 & 마케팅 안내 초안 작성"]
    I --> J["가격 분석/재고/마케팅 요약 PDF 보고서 생성"]
    K["store-managers@ 최종 승인 및 리포트 발송"]
```

#### 워크플로우 빌더 실습 단계
1. **에이전트 생성**: New Agent → Workflow Agent 선택 (`Price & Margin Optimizer`)
2. **트리거 설정**: Schedule Trigger (`Every 1 hour`)
3. **알림 수신 노드**: `subject:"Competitor Price Alert"`
4. **조건 분기**: Rules-based Branch (알림 없음 → Stop Workflow)
5. **사내 데이터 커넥터**: 재고/원가 마진 엑셀 또는 DB 연동
...
## 2.4-2. Workflow Agent - 2: 실시간 리테일 트렌드 모니터링 & 팝업 기획 파이프라인

> **제로-셋업(Zero-Setup) 엔터프라이즈 환경 특화**: 사내 데이터나 Google Workspace(GWS) 연동 없이도, 실시간 외부 웹 검색(Web Grounding)만으로 **트렌드 자동 수집 ➔ 화제성 분석 ➔ 조건부 실행 분기 ➔ Human-in-the-Loop 관리자 승인 ➔ 3대 멀티 산출물**을 전자동으로 처리하는 실무 워크플로우입니다.
...
3. **스케줄 트리거 설정 및 우측 패널에서 산출물 고도화**:
   - **3-1. 정기 실행 스케줄 지정**:
   ...
   - **3-2. 우측 패널에서 노드 프롬프트 직접 수정 (가장 확실하고 빠른 방법)**:
     - 가운데 **[맞춤형 3종 산출물 생성]** 노드를 클릭하면, **우측 패널에 `📝 요청 사항` 편집창**이 열립니다. 대화창을 거치지 않고 여기에 원하는 문구(예: *"마케팅 홍보용 문구에 2030 세대 타겟의 인스타그램 해시태그 5개 및 카카오톡 알림톡 문구 추가"*)를 직접 타이핑하거나 붙여넣으시면 즉시 반영됩니다.

   ![Workflow 에이전트 한국어 UI 상세 화면](./img/workflowagent-kr-ui.png)

   > 💡 **대화창에 자꾸 '보안 정책 위반'이 뜨는 이유**:
   > 대화창 상단에 한번 위반 경고가 발생하면 해당 대화 스레드 전체가 플래그(오염)되어 어떤 후속 질문을 넣어도 위반으로 차단됩니다. 대화창 우측 상단의 `🔄 (새로고침)` 아이콘을 눌러 새 대화를 열거나, 위 3-2번처럼 **우측 패널 `요청 사항`에서 직접 텍스트를 수정**하는 것이 가장 깔끔합니다.
