...
## 2.3. Agent Designer (노코드 맞춤형 에이전트 제작)

코드 없이 자연어 지시어와 플로우 캔버스를 통해 대화형 **2.3.1 채팅에이전트**와 다단계 비즈니스 자동화 파이프라인인 **2.3.2 워크플로에이전트**를 직접 조립하고 팀원들과 공유합니다.

---

### 2.3.1. 채팅에이전트

대화형 인터페이스를 통해 시스템 프롬프트(페르소나, 규칙)와 입출력 형식을 정의하여 업무 전용 챗 에이전트를 제작합니다.
...
Gemini Enterprise의 **워크플로(Workflow)**는 개별 프롬프트 기반의 단발성 대화를 넘어, 정해진 비즈니스 규칙과 작업 순서에 따라 **다단계 노드(Node)를 시각적으로 조합하여 복잡한 업무 프로세스를 자동 완결하는 노코드 파이프라인 시스템**입니다.  
스케줄 트리거를 통한 백그라운드 무인 실행, 특화 에이전트 간 데이터 전달, 조건 분기(Condition), 관리자 승인(Approval), 외부 서비스 연동(Email, Jira 등)까지 엔터프라이즈 업무 흐름 전체를 유기적으로 연결합니다.

#### 1) 워크플로의 핵심 개념 및 특징
- **다단계 노드 오케스트레이션 (Multi-step Orchestration)**: 데이터 수집, AI 분석, 포맷팅 등 각 업무 단계를 모듈화된 노드로 배치하고, 선행 단계의 결과를 후속 노드의 입력값으로 정확히 전달합니다.
- **조건부 분기 및 라우팅 (Conditional Branching)**: 분석 결과나 데이터 유효성에 따라 Yes/Else 경로로 동적 분기하여 유연한 비즈니스 처리를 수행합니다.
- **사람의 검토 및 승인 (Human-in-the-Loop Approval)**: 시스템 변경이나 외부 고객 발송 등 민감한 작업 직전에 관리자의 승인(Approval) 노드를 배치하여 강력한 거버넌스와 품질을 통제합니다.
- **정기 스케줄 무인 실행 & 수신함 납품 (Schedule & Inbox)**: 매일/매주 지정된 시간에 백그라운드에서 자동 실행되며, 완성된 산출물은 좌측 메뉴의 **[수신함]**에 자동 보관되어 언제든 확인 및 공유할 수 있습니다.

---

#### 💡 워크플로우 실무 설계 샘플 (Real-world Samples)

##### [Sample 1] 글로벌 고객 피드백 분석 및 신제품 개선 워크플로우
정기 스케줄에 따라 글로벌 VoC를 수집·분석하고, 유의미한 피드백에 대해 조건부로 유관부서에 이메일을 발송하는 자동화 파이프라인입니다.

![[Sample] 글로벌 고객 피드백 분석 및 신제품 개선 워크플로우](./img/workflow-sample-voc.png)

- **파이프라인 흐름**: `Schedule(스케줄)` ➔ `글로벌 VOC 전문가(Agent)` ➔ `VoC 분석 결과 검토` ➔ `Condition(조건 분기)` ➔ `[Yes: 고객 피드백 반영(Gmail) / Else: Greeting Agent]`
- **핵심 특징**:
  - Gemini Agent로 글로벌 고객 피드백 검토 에이전트 생성
  - VoC 분석 결과를 검토하여 메일 전송 여부를 판단

##### [Sample 2] 마켓 트렌드 분석 기반 재고품 판매 전략 수립
시장 트렌드를 분석하고 관리자 승인과 실제 재고 확인을 거쳐 리세일 전략 및 안내 메일을 발송하는 엔터프라이즈 다단계 워크플로우입니다.

![[Sample] 마켓 트렌드 분석 기반 재고품 판매 전략 수립](./img/workflow-sample-retail.png)

- **파이프라인 흐름**: `Daily Relaunch Trigger` ➔ `Market Research Agent` ➔ `Approval(승인)` ➔ `Inventory Matcher` ➔ `Inventory Presence Check(분기)` ➔ `Product Strategy Agent` ➔ `Email Content Formatter` ➔ `Dev Team Notification(Gmail/Jira)`
- **핵심 프로세스**:
  1. 최신 가구 시장 트렌드를 파악
  2. 재고 현황 파악하여 리브랜딩 전략 수립
  3. 재판매 마진 전략 수립
  4. 메일 또는 Jira 티켓 생성
...
2. **워크플로우 설계 프롬프트 입력**:
   ...
   ```markdown
   실시간 웹 검색을 기반으로 리테일 트렌드를 분석하고 맞춤형 보고서를 작성하는 순차 워크플로우 파이프라인을 생성해줘.

   이 워크플로는 아래 2개의 개별 노드가 순차적으로 연결된 구조로 구성해줘:

   [1단계 노드: 최신 트렌드 분석 및 브랜드 선정]
   - 유형: 생성 (Google Search 웹 검색 연동)
   - 역할: 성수, 한남 등 최신 팝업스토어 및 디저트 F&B 트렌드를 실시간 검색하여 백화점 팝업 유치 적합도가 가장 높은 1개 브랜드를 선정하고 핵심 분석 결과를 정리한다.

   [2단계 노드: 맞춤형 3종 산출물 생성]
   - 유형: 생성 (1단계 노드의 출력을 입력으로 전달받음)
   - 역할: 1단계에서 선정된 브랜드와 분석 데이터를 바탕으로 아래 3가지 산출물을 작성한다.
   ...
   > [!TIP]
   > **실습 안내: 노드 구조 확인 및 분리 보정 방법**  
   > Gemini Enterprise 워크플로 빌더의 응답 특성에 따라 2개 단계가 단일 노드로 병합되어 생성될 수 있습니다. 캔버스 화면에 **2개의 노드가 분리되어 나타나지 않은 경우**, 아래 방법 중 하나를 선택하여 파이프라인 구조를 정돈할 수 있습니다:
   > - **대화형 수정 (권장)**: 화면 좌측 하단 대화창(*무엇을 바꾸고 싶으신가요?*)에 `"트렌드 분석 노드 뒤에 '맞춤형 3종 산출물 생성' 노드를 추가해줘"`라고 입력하시면 에이전트 빌더가 자동으로 2번째 노드를 생성하여 연결합니다.
   > - **캔버스 직접 추가**: 첫 번째 노드 하단의 **`⊕` (추가)** 버튼을 클릭한 후 **[생성]**을 선택하여 노드를 추가하고, 노드 명칭을 `맞춤형 3종 산출물 생성`으로 지정합니다.
...
3. **스케줄 트리거 설정 및 산출물 고도화**:
   ...
   - **3-2. 대화창 프롬프트를 통한 기능 고도화**:
     - 화면 좌측 하단의 **`[무엇을 바꾸고 싶으신가요?]`** 대화창에 아래 요청문을 복사하여 입력하고 전송(`↑`)합니다. Gemini Enterprise가 산출물 노드에 요청 사항을 반영하여 자동으로 업데이트합니다:

     ```markdown
     마케팅 홍보용 문구에 2030 세대 타겟의 인스타그램 해시태그 5개 및 카카오톡 알림톡 문구를 추가해줘.
     ```
...
6. **최종 실행 결과 확인 (좌측 사이드바 [수신함] 메뉴)**:
   - 워크플로우가 일정에 맞춰 자동 실행되거나 수동 실행이 완료되면, **화면 좌측 사이드바 메뉴의 `[수신함]`**에서 최종 실행 결과를 확인할 수 있습니다.
   - 수신함 목록에서 해당 워크플로 항목(예: *리테일 트렌드 분석 및 팝업 전략 수립 워크플로*)을 클릭하면 생성된 3대 맞춤 산출물(경영진 브리핑, 실무 체크리스트, 마케팅 홍보 문구)의 전문을 한눈에 열람하고 업무에 즉시 활용할 수 있습니다.

   ![Gemini Enterprise 수신함에서 워크플로 최종 실행 결과 확인](./img/workflowagent-inbox-result.png)
